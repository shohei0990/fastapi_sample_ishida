...
def search_product(barcode: str): # パラメータの型を str に変更  ' OR 1=1; -- などの型を入力できる。
    print(f"User input: {barcode}")# ユーザーからの入力をログに記録
    ...
    # これは非常に危険な例です。実際のコードでは使用しないでください。
    unsafe_query = f"SELECT PRD_NAME, PRD_PRICE FROM m_product WHERE PRD_CD = '{barcode}'"
    cursor.execute(unsafe_query)

    #result = cursor.fetchone()  # 結果を1行取得
    result = cursor.fetchall()   # 結果をすべて取得
